...
// Detect if viewer (no edit param in URL)
function isViewMode() {
...
export default function App() {
  ...
  function getDayData(d) {
    return allData[d.toDateString()] || defaultDay(d.getDay());
  }
  ...
  function openModal(d) {
    ...
    setForm({ loc1: "office", loc2: "remote", ...day });
  }

  function saveModal() {
    if (!modal) return;
    const updated = { ...allData, [modal.key]: { ...form } };
    setAllData(updated);
    saveAll(updated);
    setModal(null);
    showToast("Збережено ✓");
  ...
  function copyShare() {
    navigator.clipboard.writeText(getShareUrl()).then(() => showToast("Посилання скопійовано! 🔗"));
    setShowShare(false);
  }
    ...
    <div style={{ minHeight: "100vh", background: "#0d0f14", color: "#e8eaf2", fontFamily: "'Manrope', sans-serif", paddingBottom: 40 }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        input[type=time] { background:#1c2030; border:1px solid #252a3a; border-radius:10px; padding:10px 12px; color:#e8eaf2; font-family:'Manrope',sans-serif; font-size:15px; outline:none; width:100%; -webkit-appearance:none; }
      ...
      {toast && (
        <div style={{ position:"fixed", top:20, left:"50%", transform:"translateX(-50%)", background:"#252a3a", color:"#5fffd6", padding:"10px 20px", borderRadius:99, fontSize:13, fontWeight:600, zIndex:200, whiteSpace:"nowrap" }}>
          {toast}
        </div>
      )}

      {/* Header */}
      <div style={{ background:"#161920", borderBottom:"1px solid #252a3a", padding:"16px 16px 12px", position:"sticky", top:0, zIndex:10 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {!viewOnly && (
              <>
                <button onClick={() => setShowShare(true)} style={{ padding:"7px 14px", borderRadius:99, border:"1px solid #252a3a", background:"#1c2030", color:"#5fffd6", fontFamily:"'Manrope',sans-serif", fontWeight:600, fontSize:12, cursor:"pointer" }}>
                  🔗 Поділитись
                </button>
                <button onClick={() => setEditMode(e => !e)} style={{ padding:"7px 14px", borderRadius:99, border:"none", cursor:"pointer", fontFamily:"'Manrope',sans-serif", fontWeight:600, fontSize:12, background:editMode?"#6c8eff":"#1c2030", color:editMode?"#fff":"#6b7394" }}>
                  {editMode ? "✏️ Редагую" : "✏️ Редагувати"}
                </button>
              </>
            )}
            {viewOnly && (
              <div style={{ padding:"6px 14px", borderRadius:99, background:"rgba(95,255,214,0.1)", border:"1px solid rgba(95,255,214,0.25)", color:"#5fffd6", fontSize:12, fontWeight:600 }}>
                👁 Перегляд
              </div>
            )}
          </div>
      ...
      <div style={{ padding:"16px 16px 0" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:16 }}>
          {[
            { num:`${totalH}г`, label:"Годин", color:"#6c8eff" },
            { num:officeDays, label:"Офіс / дн.", color:"#6c8eff" },
            { num:remoteDays, label:"Дистанційно", color:"#5fffd6" },
          ].map((s, i) => (
            ...
            return (
              <div key={i} onClick={() => openModal(d)} style={{ background:"#1c2030", border:`1px solid ${today ? "#6c8eff" : "#252a3a"}`, borderRadius:16, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, opacity:isWE ? 0.4 : 1, cursor:clickable ? "pointer" : "default", position:"relative", overflow:"hidden" }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  {day.type !== "absent" && day.type !== "split" && (
                    <div style={{ fontSize:12, color:"#e8eaf2", marginTop:5 }}>{day.start1} – {day.end1}</div>
                  )}
                  {day.type === "split" && (
                ...
                {h > 0 && (
                  <div style={{ textAlign:"right", minWidth:36 }}>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:16, fontWeight:700, color:cfg.color }}>{h}</div>
                    <div style={{ fontSize:9, color:"#6b7394", textTransform:"uppercase" }}>год</div>
                  </div>
                )}
      ...
      {showShare && (
        <div onClick={() => setShowShare(false)} style={{ position:"fixed", inset:0, background:"rgba(13,15,20,0.9)", backdropFilter:"blur(8px)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:"#1c2030", borderRadius:24, border:"1px solid #252a3a", padding:28, width:"100%", maxWidth:400 }}>
            ...
            <div style={{ fontSize:13, color:"#6b7394", marginBottom:20 }}>Колеги побачать твій графік у режимі перегляду — без можливості редагувати.</div>
            <div style={{ background:"#161920", border:"1px solid #252a3a", borderRadius:10, padding:"10px 14px", fontSize:12, color:"#6b7394", wordBreak:"break-all", marginBottom:16 }}>
              {getShareUrl()}
            </div>
            <button onClick={copyShare} style={{ width:"100%", padding:14, borderRadius:14, border:"none", background:"#6c8eff", color:"#fff", fontFamily:"'Unbounded',sans-serif", fontWeight:600, fontSize:14, cursor:"pointer" }}>
        ...
        <div onClick={e => { if (e.target === e.currentTarget) setModal(null); }} style={{ position:"fixed", inset:0, background:"rgba(13,15,20,0.9)", backdropFilter:"blur(8px)", zIndex:100, display:"flex", alignItems:"flex-end" }}>
          <div style={{ background:"#1c2030", borderRadius:"24px 24px 0 0", border:"1px solid #252a3a", padding:"24px 20px 40px", width:"100%", maxHeight:"90vh", overflowY:"auto" }}>
            ...
            {form.type !== "absent" && (
              <>
                <div style={{ marginBottom:16 }}>
                  ...
                  {form.type === "split" && (
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                      {["office","remote"].map(loc => { const a=(form.loc1||"office")===loc; return <button key={loc} onClick={() => setForm(f=>({...f,loc1:loc}))} style={{ padding:"9px 8px", borderRadius:10, border:`1px solid ${a?LOC[loc].color:"#252a3a"}`, background:a?LOC[loc].bg:"#161920", color:a?LOC[loc].color:"#6b7394", fontWeight:600, fontSize:12, cursor:"pointer" }}>{LOC[loc].emoji} {LOC[loc].label}</button>; })}
                    </div>
                  )}
                </div>
                {form.type === "split" && (
            ...
            <div style={{ marginBottom:24 }}>
              ...
              <textarea value={form.note||""} onChange={e => setForm(f=>({...f,note:e.target.value}))} placeholder="Напр.: нарада з 10:00, повертаюся після обіду..." />
            </div>

            <button onClick={saveModal} style={{ width:"100%", padding:16, borderRadius:14, border:"none", background:"#6c8eff", color:"#fff", fontFamily:"'Unbounded',sans-serif", fontWeight:600, fontSize:14, cursor:"pointer" }}>
              Зберегти
            </button>
          </div>
        </div>
