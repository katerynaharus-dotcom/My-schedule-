...
function calcEntryTotal(entry) {
  if (entry.manualHours) return parseFloat(entry.manualHours) || 0;
  const slots = entry.slots || [];
  if (slots.length === 0 && entry.start && entry.end) return calcDuration(entry.start, entry.end);
  return slots.reduce((s, sl) => s + calcDuration(sl.start, sl.end), 0);
}
...
function getQuarter(date) {
  return Math.floor(new Date(date).getMonth() / 3);
}

const EMPTY_SLOT = { start: "09:00", end: "13:00" };
const EMPTY_FORM = {
...
export default function Worklog({ entries, projects, onAdd, onEdit, onDelete, viewOnly }) {
  ...
  const [reportPeriod, setReportPeriod] = useState("week"); // day | week | month | quarter

  function openAdd() {
    setEditEntry(null);
    setForm({ ...EMPTY_FORM, date: new Date().toISOString().slice(0,10), slots: [{ ...EMPTY_SLOT }] });
    setShowModal(true);
  }
  ...
  function openEdit(entry) {
    ...
    setForm({
      ...
      useManual: !!entry.manualHours,
      manualHours: entry.manualHours || "",
      slots: entry.slots || (entry.start ? [{ start: entry.start, end: entry.end }] : [{ ...EMPTY_SLOT }]),
    });
    setShowModal(true);
  }
  ...
  function handleSave() {
    ...
    const item = {
      ...
      manualHours: form.useManual ? form.manualHours : "",
      slots: form.useManual ? [] : form.slots,
    };
  ...
  function addSlot() { setForm(f => ({ ...f, slots: [...f.slots, { ...EMPTY_SLOT }] })); }
  function removeSlot(i) { setForm(f => ({ ...f, slots: f.slots.filter((_,idx) => idx !== i) })); }
  ...
  const reportData = useMemo(() => {
    ...
    if (reportPeriod === "day") {
      ...
      periodEntries = entries.filter(e => {
        const d = new Date(e.date + "T00:00:00");
        return d.getFullYear() === year && getQuarter(d) === q;
      });
    }

    const totalHours = periodEntries.reduce((s,e) => s + calcEntryTotal(e), 0);
    ...
    const projectStats = Object.entries(byProject).map(([id, hours]) => {
      ...
      return {
        id,
        name: proj ? proj.name : "Без проєкту",
        color: proj ? proj.color : "#6b7394",
        bg: proj ? proj.bg : "rgba(107,115,148,0.2)",
    ...
    const byDay = {};
    periodEntries.forEach(e => {
      byDay[e.date] = (byDay[e.date] || 0) + calcEntryTotal(e);
    });

    return { totalHours: Math.round(totalHours*10)/10, totalTasks, projectStats, byDay, periodEntries };
  }, [entries, projects, reportPeriod]);

  const PERIOD_LABELS = { day:"Сьогодні", week:"Тиждень", month:"Місяць", quarter:"Квартал" };
    ...
    <div>
      ...
      <div style={{ display:"flex", gap:0, marginBottom:20, background:"#161920", borderRadius:12, padding:4 }}>
        {[{id:"log",label:"📋 Журнал"},{id:"report",label:"📊 Звіт"}].map(t => (
          <button key={t.id} onClick={()=>setTab(t.id)} style={{ flex:1, padding:"9px 0", border:"none", borderRadius:10, background:tab===t.id?"#1c2030":"transparent", color:tab===t.id?"#e8eaf2":"#6b7394", fontFamily:"'Manrope',sans-serif", fontWeight:600, fontSize:13, cursor:"pointer", transition:"all .2s" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === "log" && (
        <>
          ...
          {sortedDates.map(date => {
            ...
            const dayTotal = dayEntries.reduce((s,e) => s + calcEntryTotal(e), 0);
            return (
              <div key={date} style={{ marginBottom:20 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  {dayEntries.sort((a,b) => (a.slots?.[0]?.start||"").localeCompare(b.slots?.[0]?.start||"")).map(entry => {
                    const proj = projects.find(p => p.id === entry.projectId);
                    const dur = calcEntryTotal(entry);
                    const slots = entry.slots || (entry.start ? [{start:entry.start,end:entry.end}] : []);
                    return (
                      <div key={entry.id} style={{ background:"#1c2030", border:"1px solid #252a3a", borderRadius:14, padding:"12px 16px", position:"relative", overflow:"hidden", cursor:!viewOnly?"pointer":"default" }}
                        ...
                        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8 }}>
                          <div style={{ flex:1, paddingLeft: proj ? 6 : 0 }}>
                            ...
                            <div style={{ fontSize:11, color:"#6b7394", marginTop:5, display:"flex", flexWrap:"wrap", gap:8 }}>
                              {entry.manualHours ? (
                                <span>⏱ {entry.manualHours}г (вручну)</span>
                              ) : slots.map((sl,i) => (
                                <span key={i}>{sl.start}–{sl.end}</span>
                              ))}
                              <span style={{ color:"#5fffd6", fontWeight:600 }}>{Math.round(dur*10)/10}г</span>
                            </div>
                          </div>
                          {!viewOnly && (
        ...
        <>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:20 }}>
            {Object.entries(PERIOD_LABELS).map(([id,label]) => (
              <button key={id} onClick={()=>setReportPeriod(id)} style={{ padding:"8px 16px", borderRadius:99, border:`1px solid ${reportPeriod===id?"#6c8eff":"#252a3a"}`, background:reportPeriod===id?"rgba(108,142,255,0.13)":"#1c2030", color:reportPeriod===id?"#6c8eff":"#6b7394", fontSize:12, fontWeight:600, cursor:"pointer" }}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20 }}>
            <div style={{ background:"#1c2030", border:"1px solid #252a3a", borderRadius:14, padding:"16px", textAlign:"center" }}>
          ...
          {reportData.projectStats.length > 0 && (
            <div style={{ background:"#1c2030", border:"1px solid #252a3a", borderRadius:14, padding:"16px", marginBottom:20 }}>
              <div style={{ fontSize:11, fontWeight:600, color:"#6b7394", textTransform:"uppercase", letterSpacing:".7px", marginBottom:14 }}>По проєктах</div>
                  ...
                  <div key={p.id}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                      ...
                      <div style={{ display:"flex", gap:12 }}>
                        ...
                        <span style={{ fontSize:12, color:"#6b7394" }}>{p.pct}%</span>
                      </div>
                    </div>
                    <div style={{ height:6, background:"#161920", borderRadius:99, overflow:"hidden" }}>
                      <div style={{ height:"100%", width:`${p.pct}%`, background:p.color, borderRadius:99, transition:"width .4s" }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {Object.keys(reportData.byDay).length > 0 && (
            <div style={{ background:"#1c2030", border:"1px solid #252a3a", borderRadius:14, padding:"16px" }}>
              <div style={{ fontSize:11, fontWeight:600, color:"#6b7394", textTransform:"uppercase", letterSpacing:".7px", marginBottom:14 }}>По днях</div>
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          ...
          {reportData.totalTasks === 0 && (
            <div style={{ textAlign:"center", padding:"40px 0", color:"#3a3f52", fontSize:13 }}>Немає записів за цей період</div>
          )}
        </>
      )}
          ...
          <div style={{ background:"#1c2030", borderRadius:"24px 24px 0 0", border:"1px solid #252a3a", padding:"24px 20px 40px", width:"100%", maxHeight:"92vh", overflowY:"auto" }}>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:14 }}>
                <button onClick={()=>setForm(f=>({...f,useManual:false}))} style={{ padding:"10px 8px", borderRadius:12, border:`1px solid ${!form.useManual?"#6c8eff":"#252a3a"}`, background:!form.useManual?"rgba(108,142,255,0.13)":"#161920", color:!form.useManual?"#6c8eff":"#6b7394", fontSize:12, fontWeight:600, cursor:"pointer" }}>⏰ Проміжки</button>
                <button onClick={()=>setForm(f=>({...f,useManual:true}))} style={{ padding:"10px 8px", borderRadius:12, border:`1px solid ${form.useManual?"#5fffd6":"#252a3a"}`, background:form.useManual?"rgba(95,255,214,0.10)":"#161920", color:form.useManual?"#5fffd6":"#6b7394", fontSize:12, fontWeight:600, cursor:"pointer" }}>✏️ Вручну (г)</button>
              </div>

              {form.useManual ? (
                <div>
                  <input type="number" min="0" max="24" step="0.5" value={form.manualHours} onChange={e=>setForm(f=>({...f,manualHours:e.target.value}))} placeholder="Напр.: 2.5" style={{ ...DI, width:"100%" }} />
                  <div style={{ fontSize:11, color:"#6b7394", marginTop:6 }}>Введи кількість годин числом</div>
                </div>
              ) : (
                <div>
                  {form.slots.map((sl, i) => (
                    <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 20px 1fr auto", alignItems:"center", gap:8, marginBottom:10 }}>
                      ...
                      {form.slots.length > 1 && <button onClick={()=>removeSlot(i)} style={{ background:"transparent", border:"none", color:"#3a3f52", fontSize:18, cursor:"pointer" }}>×</button>}
                    </div>
                  ))}
                  <button onClick={addSlot} style={{ width:"100%", padding:"9px", borderRadius:10, border:"1px dashed #252a3a", background:"transparent", color:"#6b7394", fontSize:12, fontWeight:600, cursor:"pointer", marginTop:4 }}>+ Додати проміжок</button>
                  {form.slots.length > 0 && (
                    <div style={{ fontSize:12, color:"#5fffd6", marginTop:8 }}>
                      ⏱ Разом: {Math.round(form.slots.reduce((s,sl)=>s+calcDuration(sl.start,sl.end),0)*10)/10} год
                    </div>
                  )}
                </div>
              )}
